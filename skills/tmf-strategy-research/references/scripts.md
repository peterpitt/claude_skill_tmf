...
> 抓最新 TXF **1 分 K**、自動比對現有 CSV 週期取樣（1 分檔保持 1 分、5 分檔保持 5 分）、
> 增量合併去重進同一個 CSV，寫檔前自動備份 `.bak`。跑完直接重測即為最新。
> **持續自動化**：把 `update_kbars.bat` 加進 Windows「工作排程器」每天跑一次（建議夜盤收盤後、
> 約每日 06:00），資料就會自己保持最新，不必記得手動跑。紀錄見 `update_kbars.log`。
