...
- **趨勢品質濾網**（回測後新增，大幅降低尾部風險）：
  - **站穩均線**：突破需超過 `entry_ma_buffer_points`（預設 3 點）才算數，濾掉貼線雜訊。
  - **均線斜率**：多單只在 MA 上揚時進、空單只在 MA 下彎時進（`use_ma_slope_filter`）。
- **出場**：均線反向、單筆停損（預設 20 點）、或收盤前強制平倉。
- **實際下單**：訊號確認後對 **TMF/MXF 當月**送出**範圍市價（IOC）**，平倉用市價確保出得掉。

> 📊 **回測方法與最佳化結論見 [`BACKTEST.md`](BACKTEST.md)**。重點：本環境無法取得真實
> 行情，故以合成市場驗證引擎與參數穩健性；趨勢濾網把最壞情況從 −16,940 砍到 −3,660、
> 交易次數砍半。**真實邊際請務必用你自己的 K 線（`python backtest.py --csv ...`）重跑驗證。**
